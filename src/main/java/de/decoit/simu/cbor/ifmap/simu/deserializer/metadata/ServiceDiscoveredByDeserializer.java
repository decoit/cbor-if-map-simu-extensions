...
/**
 * Deserializer class to create {@link CBORServiceDiscoveredBy} instances from CBOR data.
 *
 * @author Thomas Rix (rix@decoit.de)
 */
@Slf4j
public class ServiceDiscoveredByDeserializer implements VendorMetadataDeserializer<CBORServiceDiscoveredBy> {
	private static ServiceDiscoveredByDeserializer instance;
	 ...
	 */
	public static ServiceDiscoveredByDeserializer getInstance() {
		if(instance == null) {
			instance = new ServiceDiscoveredByDeserializer();
		}
	 ...
	 */
	private ServiceDiscoveredByDeserializer() {}
	
	
	@Override
	public CBORServiceDiscoveredBy deserialize(final Array attributes, 
		...
		for(int i=0; i<attributesDataItems.size(); i=i+2) {
			...
			switch(attrNameStr) {
				case CBORServiceDiscoveredBy.IFMAP_PUBLISHER_ID:
					publisherId = MetadataDeserializerManager.processUnicodeStringItem(attrValue, true);
					break;
				case CBORServiceDiscoveredBy.IFMAP_TIMESTAMP:
					timestampDi = attrValue;
					break;
				case CBORServiceDiscoveredBy.IFMAP_TIMESTAMP_FRACTION:
		...
		CBORServiceDiscoveredBy rv;
		if(publisherId != null && timestamp != null) {
			rv = new CBORServiceDiscoveredBy(publisherId, timestamp);
		}
		else {
			rv = new CBORServiceDiscoveredBy();
		}
