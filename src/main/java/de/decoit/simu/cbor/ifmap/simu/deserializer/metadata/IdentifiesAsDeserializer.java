...
/**
 * Deserializer class to create {@link CBORIdentifiesAs} instances from CBOR data.
 *
 * @author Thomas Rix (rix@decoit.de)
 */
@Slf4j
public class IdentifiesAsDeserializer implements VendorMetadataDeserializer<CBORIdentifiesAs> {
	private static IdentifiesAsDeserializer instance;
	 ...
	 */
	public static IdentifiesAsDeserializer getInstance() {
		if(instance == null) {
			instance = new IdentifiesAsDeserializer();
		}
	 ...
	 */
	private IdentifiesAsDeserializer() {}
	
	
	@Override
	public CBORIdentifiesAs deserialize(final Array attributes, 
		...
		for(int i=0; i<attributesDataItems.size(); i=i+2) {
			...
			switch(attrNameStr) {
				case CBORIdentifiesAs.IFMAP_PUBLISHER_ID:
					publisherId = MetadataDeserializerManager.processUnicodeStringItem(attrValue, true);
					break;
				case CBORIdentifiesAs.IFMAP_TIMESTAMP:
					timestampDi = attrValue;
					break;
				case CBORIdentifiesAs.IFMAP_TIMESTAMP_FRACTION:
		...
		if(publisherId != null && timestamp != null) {
			rv = new CBORIdentifiesAs(publisherId, timestamp);
		}
		else {
			rv = new CBORIdentifiesAs();
		}
