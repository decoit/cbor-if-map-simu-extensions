...
/**
 * Deserializer class to create {@link CBORServiceImplementation} instances from CBOR data.
 *
 * @author Thomas Rix (rix@decoit.de)
 */
@Slf4j
public class ServiceImplementationDeserializer implements VendorMetadataDeserializer<CBORServiceImplementation> {
	private static ServiceImplementationDeserializer instance;
	 ...
	 */
	public static ServiceImplementationDeserializer getInstance() {
		if(instance == null) {
			instance = new ServiceImplementationDeserializer();
		}
	 ...
	 */
	private ServiceImplementationDeserializer() {}
	
	
	@Override
	public CBORServiceImplementation deserialize(final Array attributes, 
		...
		for(int i=0; i<attributesDataItems.size(); i=i+2) {
			...
			switch(attrNameStr) {
				case CBORServiceImplementation.IFMAP_PUBLISHER_ID:
					publisherId = MetadataDeserializerManager.processUnicodeStringItem(attrValue, true);
					break;
				case CBORServiceImplementation.IFMAP_TIMESTAMP:
					timestampDi = attrValue;
					break;
				case CBORServiceImplementation.IFMAP_TIMESTAMP_FRACTION:
		...
		CBORServiceImplementation rv;
		if(publisherId != null && timestamp != null) {
			rv = new CBORServiceImplementation(publisherId, timestamp);
		}
		else {
			rv = new CBORServiceImplementation();
		}
