...
/**
 * Deserializer class to create {@link CBORFileMonitored} instances from CBOR data.
 *
 * @author Thomas Rix (rix@decoit.de)
 */
@Slf4j
public class FileMonitoredDeserializer implements VendorMetadataDeserializer<CBORFileMonitored> {
	private static FileMonitoredDeserializer instance;
	 ...
	 */
	public static FileMonitoredDeserializer getInstance() {
		if(instance == null) {
			instance = new FileMonitoredDeserializer();
		}
	 ...
	 */
	private FileMonitoredDeserializer() {}
	
	
	@Override
	public CBORFileMonitored deserialize(final Array attributes, 
		...
		for(int i=0; i<attributesDataItems.size(); i=i+2) {
			...
			switch(attrNameStr) {
				case CBORFileMonitored.IFMAP_PUBLISHER_ID:
					publisherId = MetadataDeserializerManager.processUnicodeStringItem(attrValue, true);
					break;
				case CBORFileMonitored.IFMAP_TIMESTAMP:
					timestampDi = attrValue;
					break;
				case CBORFileMonitored.IFMAP_TIMESTAMP_FRACTION:
		...
		CBORFileMonitored rv;
		if(publisherId != null && timestamp != null) {
			rv = new CBORFileMonitored(publisherId, timestamp);
		}
		else {
			rv = new CBORFileMonitored();
		}
